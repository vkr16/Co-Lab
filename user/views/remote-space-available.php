            ...
            while ($data = mysqli_fetch_assoc($result)) {
                if (isSpaceConflict($data['space_no'], $area_id, $time_start, $time_end)) { ?>
                    <label class="btn btn-danger disabled shadow-sm my-1 spaceButton">
                        <input disabled type="radio" class="shadow" required name="options" value="<?= $data['space_no'] ?>"><i class="fa-solid fa-square" style="color:<?= $data['color_code'] ?>"></i> <?= $area_data['code'] . '-' . $data['space_no'] ?>
                    </label>
                <?php
                } else {
                ?>
                    <label class="btn btn-outline-success shadow-sm my-1 spaceButton">
                        <input type="radio" class="shadow" required name="options" value="<?= $data['space_no'] ?>"><i class="fa-solid fa-square" style="color:<?= $data['color_code'] ?>"></i> <?= $area_data['code'] . '-' . $data['space_no'] ?>
                    </label>
            <?php
                }
            }
